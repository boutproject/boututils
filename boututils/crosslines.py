...
def linelineintersect(a, b, atol=1e-08):
    ...
    def meshgrid_as_strided(x, y, mask=None):
        ...
        if mask is None:
            return (
                as_strided(x, strides=(0, x.strides[0]), shape=(y.size, x.size)),
    ...
    denom_[:, :] = np.where(denom_ == 0.0, 1.0e-100, denom_)
...
def find_inter(contour1, contour2):
    ...
    i = 0
    ncombos = sum([len(x.get_paths()) for x in contour1.collections]) * sum(
        [len(x.get_paths()) for x in contour2.collections]
    )
    for linecol1, linecol2 in itertools.product(
        contour1.collections, contour2.collections
    ):
        for path1, path2 in itertools.product(
            linecol1.get_paths(), linecol2.get_paths()
        ):
            i += 1
